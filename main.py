...
current_workding_dir = pathlib.Path(__file__).parent
config_path = current_workding_dir / CONFIG_FILENAME
...
def setup_logging():
    ...
    rfh = logging.handlers.RotatingFileHandler(
        "logs/superior-spork.log", maxBytes=max_bytes, backupCount=10
    )
...
class Minecord(commands.Bot):
    def __init__(self) -> None:
        super().__init__(
            ...
            status=Status.dnd,
            activity=Activity(
                type=ActivityType.watching, name="Mining blocks || m!help"
            ),
